...
- 最小副作用文件约束：每次外部运行都会先在系统临时目录下创建独立的 `rflux-ext-*` 子目录，把输入 deck 写入该目录。成功运行后会把 deck / waveform 复制到稳定的临时文件并清理运行目录；失败运行会保留运行目录以便复审，避免把诊断证据过早删除。
