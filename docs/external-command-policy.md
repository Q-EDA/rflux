...
- 最小副作用文件约束：每次外部运行都会先在系统临时目录下创建独立的 `rflux-ext-*` 子目录，把输入 deck 写入该目录，并在成功运行后把 deck / waveform 复制到稳定的临时文件，再清理运行目录，降低默认输出文件污染其他路径的概率。
...
- 更明确的外部输出文件 allowlist / retain policy
- 面向多外部仿真器的策略扩展机制
