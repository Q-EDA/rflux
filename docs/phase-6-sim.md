...
- deterministic replay tests for identical seeds/options, including Rust internal-transient unit coverage plus Python `simulate_text(...)` same-seed replay and different-seed divergence checks (`uv run pytest python/tests/test_basic.py -k "same_seed or different_seed"`)
- Python control-card validation now also covers `.tran` print-window capture plus `.nodeset` spaced/comma forms and `.options` space-separated alias pairs through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "print_window or spaced_and_comma_forms or options_card_space_pairs"`)
- Python measurement validation now also covers `.measure`/`.meas` scalar extraction, warning paths, and first-order delay measurements through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "measurement_details or measurement_warnings or delay_measurements or supports_meas_alias"`)
- direct simulator-crate integration is now also validated at the CLI layer: `rflux-cli` `simulate-file` consumes `rflux-sim::simulate_file(...)` and emits schema-versioned report JSON in `cargo test -p rflux-cli run_simulate_file_emits_internal_transient_report_json`
- Python independent-source validation now also covers `DC=...` and spaced `DC = ...` forms with `AC` trailers through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "dc_source_with_ac_tail or dc_equals_source_with_ac_tail or dc_spaced_equals_source_with_ac_tail"`)
- Python passive-device validation now also covers keyword and space-separated `R/L/C` value forms through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "keyword_resistor_value or keyword_capacitor_value or keyword_inductor_value or space_separated_keyword_resistor_value or space_separated_keyword_capacitor_value or space_separated_keyword_inductor_value"`)
- Python mutual-inductance validation now also covers spaced keyword and alias pair forms (`coupling 0.9`, `k 0.9`) through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "mutual_inductance and (coupling_keyword or space_separated_coupling or space_separated_k_alias or supports_mutual_inductance)"`)
- Python transmission-line validation now also covers space-separated and comma-separated `T` keyword cards plus alias and attenuation forms (`z0 50 td 3p loss 0.3`, `z0=50, td=3p, loss=0.3`, `zo=50 tau=3p atten=0.3`, `alpha=1e11`) through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "zero_delay_transmission_line_subset or finite_delay_transmission_line_subset or transmission_line_loss_parameter or spaced_t_assignments or space_separated_t_keywords or comma_separated_t_keywords or t_alias_keywords or t_alpha_attenuation"`)
- Python `.param` validation now also covers spaced/comma assignment forms, continuation-line normalization, and scientific-notation literals through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "parses_spaced_and_comma_param_assignments or parses_param_continuation_lines or parses_scientific_notation_params"`)
- Python expression-evaluation validation now also covers arithmetic parameter expressions in supported element-value contexts plus braced parameter references through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "parses_param_arithmetic_expressions or parses_braced_param_references or parses_scientific_notation_params"`)
- Python thermal/noise validation now also covers `temp`/`tnom` plus alias families (`noise_sigma`, `sigma`, `temperature`, `nominal_temperature`, `temperature_k`, `nominal_temperature_k`, `tempc`, `tnomc`) through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "accepts_option_temp_and_tnom or accepts_option_aliases_for_noise_and_temperature or accepts_kelvin_option_temperature_aliases or accepts_celsius_option_temperature_aliases or noise_is_reproducible_with_same_seed or noise_differs_for_different_seed"`)
- Python file-driven waveform validation now also covers direct deck-local `PWL(file=...)` sources and included-file `PWL(file=...)` sources through `simulate_file(...)` (`uv run pytest python/tests/test_basic.py -k "resolves_file_driven_pwl_source_from_deck_directory or resolves_included_file_driven_pwl_source"`)
- Python include/library validation now also covers `.lib <path> section = ...` section selection through `simulate_file(...)` (`uv run pytest python/tests/test_basic.py -k "resolves_lib_section_and_returns_internal_transient_report or resolves_include_and_returns_event_only_report"`)
- Python subcircuit-parameter validation now also covers nested passthrough from outer `X... params:` assignments into inner subcircuit instance arguments through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "subckt_param_override or params_marker_override or nested_subckt_param_passthrough"`)
- Python subcircuit hierarchy validation now also covers nested `.subckt` definitions and same-scope duplicate-definition diagnostics through `simulate_text(...)` (`uv run pytest python/tests/test_basic.py -k "nested_subckt_param_passthrough or supports_nested_subckt_definition or duplicate_subckt_name_in_same_scope"`)
...
- local real-JoSIM correlation evidence on Windows for both the full `josephson-junction` manifest subset and the full current phase-6 threshold manifest: `uv run python python/scripts/run_waveform_compare_manifest.py --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --category josephson-junction --result-dir target\waveform-compare-jj-full --validate-pass` completes with `failures=0` and worst observed `worst_max_abs_v = 6.710659e-04 V`, while the latest refreshed full-threshold run `uv run python python/scripts/run_waveform_compare_manifest.py --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\\waveform-compare-full-105 --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json --validate-no-regression` now also completes with `failures=0` across the current 105-deck phase-6 baseline, including 42 `josephson-junction` decks; the full-run worst observed `worst_max_abs_v = 1.5877853e-03 V` on `sin_mixed_case_source_smoke.cir`, and the worst JJ observed `worst_max_abs_v = 6.71065936427136e-04 V` on `include_chain_source_k_jj_subckt_smoke.cir`
- dedicated compact mixed-case alias source evidence now also exists in the numeric manifest flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck exp_alias_mixed_case_source_smoke.cir --deck pulse_alias_mixed_case_source_smoke.cir --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\waveform-compare-source-alias-mixed-case-compact --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json` completes with `failures=0`; `History Diff` reports both decks as `NEW -> PASS`, and worst observed `worst_max_abs_v = 1.000000e-03 V` on `pulse_alias_mixed_case_source_smoke.cir`
- dedicated `.param` smoke evidence now also exists in the numeric manifest flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck param_rc_smoke.cir --deck param_spaced_comma_rc_smoke.cir --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\waveform-compare-param-rc --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json` completes with `failures=0`; `History Diff` reports both decks as `NEW -> PASS`, and worst observed `worst_max_abs_v = 6.494191e-05 V` on `param_spaced_comma_rc_smoke.cir`
- dedicated continuation-line smoke evidence now also exists in the numeric manifest flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck continuation_rc_smoke.cir --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\waveform-compare-continuation-rc --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json` completes with `failures=0`; `History Diff` reports `continuation_rc_smoke.cir` as `NEW -> PASS`, and worst observed `worst_max_abs_v = 6.494191e-05 V`
- dedicated `.option` syntax smoke evidence now also exists in the numeric manifest flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck option_reltol_abstol_space_rc_smoke.cir --deck option_maxiter_space_rc_smoke.cir --deck options_mixed_comma_rc_smoke.cir --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\waveform-compare-option-rc --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json` completes with `failures=0`; `History Diff` reports all three decks as `NEW -> PASS`, and worst observed `worst_max_abs_v = 6.474674e-05 V` on `options_mixed_comma_rc_smoke.cir`
- dedicated option case-variant syntax smoke evidence now also exists in the numeric manifest flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck option_uppercase_reltol_abstol_space_rc_smoke.cir --deck option_mixed_case_maxiter_space_rc_smoke.cir --deck option_uppercase_comma_rc_smoke.cir --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\waveform-compare-option-case-rc --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json` completes with `failures=0`; `History Diff` reports all three decks as `NEW -> PASS`, and worst observed `worst_max_abs_v = 6.474674e-05 V` on `option_uppercase_reltol_abstol_space_rc_smoke.cir`
- dedicated SIN alias mixed-case source evidence now also exists in the numeric manifest flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck sin_alias_mixed_case_source_smoke.cir --deck sin_alias_mixed_case_comma_source_smoke.cir --deck sin_alias_mixed_case_space_before_paren_source_smoke.cir --josim-command "C:\tools\JoSIM-v2.7-windows-x64\bin\josim-cli.exe" --result-dir target\waveform-compare-sin-alias-mixed-case --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json` completes with `failures=0`; `History Diff` reports all three decks as `NEW -> PASS`, and worst observed `worst_max_abs_v = 1.000000e-03 V` on `sin_alias_mixed_case_space_before_paren_source_smoke.cir`
...
- dedicated phase= alias mixed-case compact keyword-form `sIn(vo=... va=... freq=... td=... theta=... phase=...)` source focused evidence now also exists in the numeric compare flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck sin_phase_alias_keyword_mixed_case_source_smoke.cir --josim-command "C:/tools/JoSIM-v2.7-windows-x64/bin/josim-cli.exe" --result-dir target/waveform-compare-sin-phase-alias-keyword-mixed-case-source --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json --validate-no-regression` reports `failures=0` with `worst_max_abs_v = 1.000000e-03 V`
- dedicated phase= alias compact keyword-form `SIN(vo=... va=... freq=... td=... theta=... phase=...)` source focused evidence now also exists in the numeric compare flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck sin_phase_alias_keyword_source_smoke.cir --josim-command "C:/tools/JoSIM-v2.7-windows-x64/bin/josim-cli.exe" --result-dir target/waveform-compare-sin-phase-alias-keyword-source --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json --validate-no-regression` reports `failures=0` with `worst_max_abs_v = 1.000000e-03 V`
...
- dedicated alias mixed-case keyword-form `eXp (low=... high=... delay1=... tau_rise=... delay2=... tau_fall=...)` source focused evidence with whitespace before `(` now also exists in the numeric compare flow: `uv run python python/scripts/run_waveform_compare_manifest.py --deck exp_alias_mixed_case_space_before_paren_source_smoke.cir --josim-command "C:/tools/JoSIM-v2.7-windows-x64/bin/josim-cli.exe" --result-dir target/waveform-compare-exp-alias-mixed-case-space-before-paren-source --validate-pass --previous-summary-json python/tests/benchmarks/phase6/waveform_compare_summary.windows-approved-baseline.json --validate-no-regression` reports `failures=0` with `worst_max_abs_v = 9.905453e-04 V`
