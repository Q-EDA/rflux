...
| Explicit backend selection | Distinct simulator execution path selection | `simulation_mode` is exposed across Rust, PyO3, and Python for `auto`, `event_only`, `external_josim`, `internal_transient`; reports now also expose machine-readable `requested_mode` alongside resolved backend so embedding/CLI consumers can verify mode-selection behavior deterministically | done | 6.0 | Backend selection contract is explicit and validated across surfaces |
...
| `.tran` options such as print-window control | Supported by JoSIM | The current supported control-card subset is implemented and cross-surface validated: `.tran` accepts optional print-window fields, trailing `uic`, keyword-style `tstep/tstop/tstart/tprint` forms (including spaced `=` spellings), `.ic` startup values apply under `uic`, `.nodeset` startup hints accept compact plus spaced/comma voltage assignments, and `.option`/`.options` accept the current supported tolerance and iteration keys including alias names plus space-separated `name value` pairs; Rust and Python tests now both validate print-window capture behavior and these supported startup/option forms | done | 6.1 / 6.2 | Current supported control-card subset is explicit and validated |
| `.measure` extraction | Supported by JoSIM | The current supported measurement subset is implemented and cross-surface validated: internal transient accepts `.measure` and `.meas` for scalar voltage `max|min|pp|avg|rms|final`, `find ... AT=...`, `find ... WHEN ...`, and first-order `TRIG`/`TARG|TARGET` delay measurements with differential probes, `RISE|FALL|CROSS`, `LAST`, and `TD`; results surface through `measurement_details` and `delay_details`, while unresolved probes, empty windows, missing crossings, unavailable samples, and non-finite results surface through `measurement_warnings`. Rust and Python tests now both validate the supported scalar, warning, and delay-measurement forms | done | 6.5 | Current supported measurement subset is explicit and validated |
...
| Deterministic RNG seed | Supported via `.option seed=` | Internal transient parses `.option seed=...` (including spaced `seed = ...`, parameter expressions, comma-separated `.option` lists, and multi-line `.option` decks where non-seed lines do not clear an earlier seed), surfaces it in result metadata, and uses it to drive reproducible internal noise sampling across the currently supported source-noise and resistor-noise paths; reproducibility is now validated in both Rust and Python entrypoints for same-seed replay and different-seed divergence | done | 6.3 | Current seeded-noise contract is explicit and cross-surface validated |
...
| Error diagnostics | Mature parser/runtime diagnostics | Parser/runtime failures now surface machine-readable diagnostic codes (`diagnostic_code`) across Rust, PyO3, Python, and CLI JSON, alongside quality-gate status and detailed warning/violation vectors | done | 6.1-6.5 | Structured diagnostics are now embeddable and contract-stable |
